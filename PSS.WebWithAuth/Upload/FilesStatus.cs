...
namespace PSS.WebWithAuth.Upload
{
    ...
    public class FilesStatus
    {
        ...
        {
            get { return Configurations.UploadHandlerRoot; }
        }
        ...
        public FilesStatus(FileInfo fileInfo, string userKey)
        {
            SetValues(userKey + "/" + fileInfo.Name, (int)fileInfo.Length, fileInfo.FullName);
        }

        // plamen: modified to handle separate user folders
        public FilesStatus(string fileName, int fileLength, string fullPath, string userKey) 
        {
            SetValues(userKey + "/" + fileName, fileLength, fullPath); 
        }
